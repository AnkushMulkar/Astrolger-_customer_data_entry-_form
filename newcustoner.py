...
COLUMNS = ['Name', 'Gender', 'Birthdate', 'Birthplace', 'Birth Time', 'Address', 'City', 'State', 'Country', 'Occupation', 'Times of Visit', 'Type of Question', 'Number of Questions', 'Password']

# Define a function to add a new customer to the Excel file
def add_customer(name, gender, birthdate, birthplace, birth_time, address, city, state, country, occupation, times_of_visit, type_of_question, number_of_questions, password):
    ...
    new_customer = pd.DataFrame([[name, gender, birthdate, birthplace, birth_time, address, city, state, country, occupation, times_of_visit, type_of_question, number_of_questions, password]], columns=COLUMNS)
...
try:
    df = pd.read_excel(EXCEL_FILE)
    st.write(df.drop('Password', axis=1))
except:
